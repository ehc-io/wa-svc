...
func (c *Client) Connect(ctx context.Context, opts ConnectOptions) error {
	...
	for {
		...
		case evt, ok := <-qrChan:
			if !ok {
				// QR channel closed - check if we're now authenticated
				// This happens after successful pairing
				if cli.Store != nil && cli.Store.ID != nil {
					return nil
				}
				return fmt.Errorf("QR channel closed without completing authentication")
			}
			switch evt.Event {
			case "code":
			...
			case "success":
				// QR was scanned, but pairing is NOT complete yet!
				// The QR channel will close after pairing completes.
				// Continue waiting for the channel to close or for auth to complete.
				// Don't return here - wait for the channel to close.
			case "timeout":
